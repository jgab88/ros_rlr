...
class MeshGenerator(Node):
    def __init__(self):
        ...
        self.full_point_cloud = o3d.geometry.PointCloud()
        self.point_cloud_lock = threading.Lock()
        self.point_collection_thread = threading.Thread(target=self.point_collection_worker)
        self.point_collection_thread.start()
    ...
    def scan_callback(self, msg):
        ...
        with self.point_cloud_lock:
            self.full_point_cloud += cloud

        self.get_logger().info(f"Accumulated {len(points)} points. Total for mesh: {len(self.accumulated_cloud.points)}, Total overall: {len(self.full_point_cloud.points)}")

        self.total_scans += 1
        if self.total_scans % self.save_interval == 0:
            self.save_complete_mesh()

    def point_collection_worker(self):
        while rclpy.ok():
            time.sleep(1)  # Sleep to prevent this thread from consuming too much CPU
    ...
    def generate_and_publish_mesh(self, mesh_id):
        ...
        try:
            ...
            radius = 3 * avg_dist  # Increased radius for better overlap
    ...
    def save_complete_mesh(self):
        ...
        try:
            ...
            # Simplify mesh
            mesh = mesh.simplify_quadric_decimation(target_number_of_triangles=200000)  # Increased for final mesh
    ...
    def save_full_point_cloud(self):
        try:
            with self.point_cloud_lock:
                cloud_copy = copy.deepcopy(self.full_point_cloud)

            filename = os.path.join(self.output_directory, "full_point_cloud.ply")
            o3d.io.write_point_cloud(filename, cloud_copy)
            self.get_logger().info(f"Full point cloud saved to {filename} with {len(cloud_copy.points)} points")
        except Exception as e:
            self.get_logger().error(f"Error saving full point cloud: {str(e)}")

    def destroy_node(self):
        # Save final mesh and full point cloud before shutting down
        self.save_complete_mesh()
        self.save_full_point_cloud()
        self.mesh_queue.put(None)  # Signal the worker thread to exit
        self.mesh_generation_thread.join()
        self.point_collection_thread.join()
